...
def feistel(R, key):
    ...
    for i in range(8):
        ...
        substituted.extend(to_bits(value, 4))

    return permute(substituted, P)


def encrypt_block(block, round_keys):
    bits = permute(to_bits(block, 64), IP)
    L = bits[:32]
    R = bits[32:]

    for key in round_keys:
        new_R = [a ^ b for a, b in zip(L, feistel(R, key))]
        L = R
        R = new_R

    combined = permute(R + L, FP)
    result = 0
    for bit in combined:
        result = (result << 1) | bit
    return result
...
def encrypt(plaintext, key):
    ...
    round_keys = generate_keys(key_value)
    ciphertext = b""

    for i in range(0, len(plaintext), 8):
        block = plaintext[i:i + 8]
        block_value = int.from_bytes(block, "big")
        encrypted = encrypt_block(block_value, round_keys)
        ciphertext += encrypted.to_bytes(8, "big")

    return ciphertext


plaintext = b"Lohith Srikar, 2024064631"
given_key = "GITAMUNIVERSITY"
des_key = given_key[:8].encode("ascii")  # First 8 characters for 64-bit key

ciphertext = encrypt(plaintext, des_key)

print("DES Encryption")
print("-----------------------------")
print("Plain Text :", plaintext.decode())
print("Given Key  :", given_key)
print("DES Key    :", des_key.decode())
